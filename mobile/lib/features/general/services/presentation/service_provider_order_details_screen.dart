...
class ServiceProviderOrderDetailsScreen extends ConsumerWidget {
  ...
  Widget _buildInfoRow(ThemeData theme, String label, String value, {Color? textColor}) {
    return Padding(
      padding: EdgeInsets.only(bottom: 0.5.h),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('$label: ', style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.primaryContainer)),
          Expanded(child: Text(value, style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600, color: textColor))),
        ],
      ),
    );
  }
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SizedBox(height: 4.h),
              NestFormField(controller: reasonController, hintText: "Enter reason", maxLines: 3, belowSpacing: false),
            ],
          ),
          actions: [
            TextButton(onPressed: () => context.pop(), child: Text("Cancel", style: theme.textTheme.bodyMedium)),
            ElevatedButton(
              onPressed: () {
                if (reasonController.text.trim().isNotEmpty) {
                  ...
                  ToastUtil.showErrorToast(context, "Please provide a reason for declining this order.");
                }
              },
              style: ElevatedButton.styleFrom(backgroundColor: Colors.red),
              child: Text("Decline", style: theme.textTheme.bodyMedium?.copyWith(color: Colors.white, fontWeight: FontWeight.w600)),
            ),
          ],
        );
      },
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: IconImageWidget(iconName: getItemIcon(), color: Colors.blue.shade700, width: 1.w, height: 1.h),
          ),
          SizedBox(width: 3.w),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('${item.category} • \$${item.price.toStringAsFixed(2)} each', style: theme.textTheme.bodySmall?.copyWith(color: Colors.grey)),
              ],
            ),
          ),
          ...
          Column(
            ...
            children: [
              Text('x${item.quantity}', style: theme.textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.bold)),
              SizedBox(height: 0.5.h),
              Text('\$${(item.price * item.quantity).toStringAsFixed(2)}', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w500)),
            ],
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _buildStatusButton("In Progress", Icons.sync, Colors.blue, order.status == "In Progress"),
                  const SizedBox(width: 8),
                  _buildStatusButton(
                    "Pickup arranged",
                  ...
                  const SizedBox(width: 8),
                  _buildStatusButton("Laundry complete", Icons.local_laundry_service_outlined, Colors.blue, order.status == "Laundry in process"),
                  const SizedBox(width: 8),
                  _buildStatusButton("Ready for Delivery", Icons.local_shipping_outlined, Colors.purple, order.status == "Ready for Delivery"),
                  const SizedBox(width: 8),
                  _buildStatusButton("Out for Delivery/Picked up", Icons.local_shipping, Colors.indigo, order.status == "Out for Delivery"),
                  const SizedBox(width: 8),
                  _buildStatusButton("Delivered", Icons.check_circle, Colors.green, order.status == "Delivered"),
                ],
              ),
            ),
          ],
  ...
  Widget _buildStatusButton(String status, IconData icon, Color color, bool isSelected) {
    return InkWell(
      onTap: () {},
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(status, style: TextStyle(color: isSelected ? Colors.white : color, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
